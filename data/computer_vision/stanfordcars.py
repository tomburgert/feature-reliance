...
class StanfordCarsDataset(Dataset):
    def __init__(self, dataset_path: str, split: str = None, transform: Optional[transforms.Compose] = None):
        ...
        if split == 'train' or 'validation':
            self.dataset = torch_datasets.StanfordCars(dataset_path, split='train')
            self.targets = np.array(list(map(lambda t: t[1], self.dataset._samples)))
            self.train_val_idx = np.arange(len(self.dataset))
            self.deterministic_train_val_split()

        if split == 'test':
            self.dataset = torch_datasets.StanfordCars(dataset_path, split='test')
            self.targets = np.array(list(map(lambda t: t[1], self.dataset._samples)))
...
class StanfordCarsDataModule(LightningDataModule):
    ...
    def setup(self, stage=None):
        if stage == 'fit' or stage is None:
            ...
            self.val_dataset = StanfordCarsDataset(
                dataset_path=self.dataset_path,
                split='validation',
